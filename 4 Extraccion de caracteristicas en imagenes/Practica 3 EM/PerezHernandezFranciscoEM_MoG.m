...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Inicialización
salida_e = zeros(size(x,1),length(lambda));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
arg(1) = sum(log(lik));
incr = 1+tol;
while incr>tol && i<maxiter
    ...
    for k=1:size(lambda,2)
        salida_e(:,k) = lambda(k)*mvnpdf(x,mu(:,k)',Sigma(:,:,k));                        
    end
    salida_e = salida_e./(repmat(sum(salida_e,2),1,size(salida_e,2)));
    ...
    % Actualizo los pesos                                      
    lambda = sum(salida_e,1)/(size(salida_e,1));                                        
                                                               
    % Actualizo las medias                                     
    mu = salida_e'*x;
    mu = mu./repmat((sum(salida_e,1))',1,size(mu,2));
                                                               
    % Actualizo las matrices de covarianza     
    for j= 1 : size(lambda,2)
        varianza= repmat(salida_e(:,j),1,size(x,2)).*(x-repmat(mu(j,:),size(x,1),1));
        Sigma(:,:,j)= (varianza'*varianza)/sum(salida_e(:,j),1);
    end
